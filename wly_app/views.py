...
#能力测试管理
def pytest_manage(request):
    pytest = BodyParts.objects.all()
    code=[]
    partname=[]
    testname=[]
    testdec=[]
    for test in pytest:
        code.append(test.partcode)
        partname.append(test.partname)
        testname.append(test.parttestname)
        testdec.append(test.parttestdesc)
    testinfo = list(zip(code,partname,testname,testdec))
    return render(request,"wly_app/pytestmanage.html",locals())

#编辑能力测试
def edittest(request,partcode):
    partcode = BodyParts.objects.filter(partcode=partcode)[0].partcode
    partname = BodyParts.objects.filter(partcode=partcode)[0].partname
    testname = BodyParts.objects.filter(partcode=partcode)[0].parttestname
    description = BodyParts.objects.filter(partcode=partcode)[0].parttestdesc
    return render(request,'wly_app/edittest.html',locals())
...
def addtest(request):
    if request.method=='POST':
        ...
        if partcode=='' or partname=='' or testname=='' or description=='':
            return render(request, 'command/test/addtest.html', {'ret': '所有选项不能为空'})
        models.BodyParts.objects.create(partcode=partcode,partname=partname,parttestname=testname,parttestdesc=description)
        messages.success(request,"提交成功")
        return redirect("/command/pytestmanage")
    else:
        return render(request,'wly_app/addtest.html')

#删除能力测试
def deletetest(request,partcode):
    models.BodyParts.objects.filter(partcode=partcode).delete()
    return redirect('/command/pytestmanage')
...
def coursemanage(request):
    ...
    dec=[]
    for each in course:
        code.append(each.courseid)
        coursename.append(each.coursename)
        dec.append(each.decription)
    rccourses = list(zip(code,coursename,dec))
    paginator = Paginator(rccourses,6)
    if request.method=="GET":
            page = request.GET.get('page')
            try:
                courses = paginator.page(page)
                ...
                courses = paginator.page(paginator.num_pages)
    return render(request,"wly_app/coursemanage.html",locals())
...
def addcourse(request):
    if request.method=='POST':
        ...
        description = request.POST.get('description')
        if courseid=='' or coursename=='' or description=='':
            return render(request, 'command/test/addtest.html', {'ret': '所有选项不能为空'})
        models.Courses.objects.create(courseid=courseid,coursename=coursename,decription=description)
        messages.success(request,"提交成功")
        return redirect("/command/coursemanage")
    else:
        return render(request,'wly_app/addcourse.html')

def deletecourse(request,courseid):
    models.Courses.objects.filter(courseid=courseid).delete()
    return redirect('/command/coursemanage')

#编辑能力测试
def editcourse(request,courseid):
    courseid = Courses.objects.filter(courseid=courseid)[0].courseid
    coursename = Courses.objects.filter(courseid=courseid)[0].coursename
    description = Courses.objects.filter(courseid=courseid)[0].decription
    return render(request,'wly_app/editcourse.html',locals())
